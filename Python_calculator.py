...
##All the calculation operations 
def add(a, b):
    return a + b
...
def select_op(choice):
    ...
    if choice == "$":
        ...
        while True:
            ...
            reset_parameter = str(first_number).find("$")  #to check $ symbol there to reset
            exit_parameter  = str(first_number).find("#")  #to check # symbol there to exit
        ...
        while (reset_parameter == -1 and exit_parameter == -1):
            ...
            reset_parameter = str(second_number).find("$") #to check $ symbol there to reset
            exit_parameter = str(second_number).find("#")  # to check # symbol there to exit
        ...
        if reset_parameter != -1:
            return 1 #  exit from the if close because of reset option $

        if exit_parameter != -1:
            return -1 # exit from the if close because of reset option $

        else:
        #operation for the given two numbers
            if choice == "+":
